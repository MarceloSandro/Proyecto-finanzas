...
def load_data(tickers, start, end):
    """
    Descarga precios de cierre y devuelve DataFrame + tickers inválidos.
    Funciona para 1 o varios tickers, evita problemas con group_by y MultiIndex.
    """
    ...
    # Caso 2: un ticker → convertir a DataFrame
    if isinstance(raw, pd.Series):
        df = raw.to_frame(name=tickers[0])
    else:
        # Caso 3: varios tickers → tomar solo 'Close' si existe
